...
My goal was to be have a Gig entry with mant 

My initial ERD was approved, but over the process of the project I would have to make many changes. 

![erdA](erdA.png)

There needed to be join tables between songs and setlist tables, but also the musicians and gig table and neither are indicated on my ERD.

My initial intention was to have a database of musicians that could be added to the gig table. Unfortunately I relaised that I'd actually only allowed for a single field. My understanding is that this would populate with only a single musician. That wasn't going to work, as I wanted it to be able to specify which combination of musicians would be performing. It's not uncommon for musicians to either have multiple bands that they work within. Some have no band at all and just "float" between gigs. To get around this I moved to a different itteration of my ERD
### Feeling
