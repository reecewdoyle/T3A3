...
The most obvious way to use an AI when coding is to use it on code that is proving difficult to debug. Looking at a very large codebase that might have gotten out of hand, or fatigue can lead to simple errors. 
...
The error in this code is that the `=` in the first `if` statement would be an assignment operator, rather than a comparison operator like it was intended. This would raise a `SyntaxError`
***
